...
correctness_file_path_new = config.PATH_RESULTS + '/CombinedResultsByTask.csv'
correctness_data_new = pd.read_csv(correctness_file_path_new)
pastel_colors_swapped = {'CM': '#77DD77', 'CP': '#FFB347'}
...
mean_correctness_per_task_new = correctness_data_new.groupby(['Task', 'Type']).mean()['Accuracy'].unstack()
...
for i, point in enumerate(X_correctness_new):
    ...
    color = 'black' if np.isclose(distance, 0) else pastel_colors_swapped['CM'] if distance > 0 else \
    pastel_colors_swapped['CP'] if distance < 0 else 'lightgrey'
...
ax.set_xlabel('Mean Correctness for Comments Missing (CM) in %', fontsize=16)
ax.set_ylabel('Mean Correctness for Comments Present (CP) in %', fontsize=16)
ax.set_xlim(0, 105)
ax.set_ylim(0, 105)
...
ax.plot([0, 105], [0, 105], color='lightgray', linestyle='--')

texts = []
# Adding task labels with increased font size
for i, point in enumerate(X_correctness_new):
    # strip 'Task' from task name
    task_number = mean_correctness_per_task_new.index[i].strip('Task')
    text = ax.text(point[0] + 1, point[1], task_number, fontsize=16, ha='left')
    texts.append(text)

adjust_text(texts)

# File path for saving with corrected axes labels and increased font size
correctness_plot_file_path_corrected_new = '/Users/Youssef/Desktop/Correctness_Scatter_Plot.pdf'
